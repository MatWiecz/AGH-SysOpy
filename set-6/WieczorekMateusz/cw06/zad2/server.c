...
char request_queue_path[64] = "";
mqd_t request_queue = -1;
...
int register_new_user(struct chat_message *message)
{
	...
	char *private_queue_path = temp_mt_init->queue_path;
	int cur_id = 0;
	...
	if (cur_id == MAX_CLIENT_CONNECTS)
	{
		fprintf(stderr, "ERROR! List for client's connections is full, so "
					"cannot register new user with queue path \"%s\"!\n",
				private_queue_path);
		//TODOok:
		mqd_t temp_queue = mq_open(private_queue_path, O_WRONLY | O_NONBLOCK);
		if (temp_queue != -1)
			send_response((int)temp_queue, message, 0);
		mq_close(temp_queue);
		return -1;
	}
	//TODOok:
	if ((client_connections.clients[cur_id].private_queue =
			 mq_open(private_queue_path, O_WRONLY | O_NONBLOCK)) == -1)
	{
		fprintf(stderr, "ERROR! Cannot register new user with private queue "
			"path \"%s\"!\n", private_queue_path);
		return -2;
	}
...
int unregister_user(struct chat_message *message)
{
	...
	mq_close(client_connections.clients[user_id].private_queue);
	client_connections.clients[user_id].private_queue = -1;
...
void atexit_action()
{
	...
	while (mq_unlink(request_queue_path));
	free_whole_mem();
}
...
int main(int argc, char *argv[])
{
	...
	char queue_name[16] = "";
	sprintf(queue_name, "/%08x", PROJECT_IDENTIFIER);
	strcat(request_queue_path, queue_name);
	request_queue = mq_open(request_queue_path,
							O_CREAT | O_EXCL | O_RDONLY | O_NONBLOCK,
							0666, &message_queue_attr);
	if (request_queue == -1)
	{
		perror("mq_open()");
