...
int private_message_queue = -1;
int server_request_queue = -1;
...
void init_client(key_t self_key)
{
#ifdef DEBUG
	printf("%d:%s()\n", getpid(), __FUNCTION__);
#endif
	struct mt_init *temp_mt_init = alloc_mt_init(self_key);
...
void atexit_action()
{
	...
	msgctl(private_message_queue, IPC_RMID, NULL);
	free_whole_mem();
}
...
int main(int argc, char *argv[])
{
	...
	key_t unique_key = ftok(home_dir_path, PROJECT_IDENTIFIER ^ getpid());
	private_message_queue = msgget(unique_key, IPC_CREAT | IPC_EXCL | 0666);
	...
	key_t server_unique_key = ftok(home_dir_path, PROJECT_IDENTIFIER);
	server_request_queue = msgget(server_unique_key, 0);
